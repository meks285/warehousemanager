...
<table border=0 cellpadding=0 cellspacing=0 width=920 style='border-collapse:
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <input style="background-color: #FFFFFF;" type="text" id="requestlocation" name="requestlocation" class="input w-100" />
  <input type="hidden" name="issuerlocation" id="issuerlocation"  value='{{ Auth::User()->recipientlocation }}' />
   </td>
  <td colspan=6 class=xl12111054 width=29 style='border-right:1.0pt solid black;border-left:none;width:248pt'>Site Reference: <input class="input w-100" style="background-color: #FFFFFF;" type="text" id="siterefnumber" name="siterefnumber" readonly /></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  <td colspan=3 class=xl12311054 style='border-right:1.0pt solid black'><input style="background-color: #FFFFFF;" type="text" id="cmsrefnumber" name="cmsrefnumber" readonly /></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
 ...
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  ...
  height:15.0pt;border-left:none'>Contact Person Name: <input name="locationcontactperson" id="locationcontactperson" type="text"  style="background-color: #FFFFFF" /></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
  <td colspan=6 height=20 class=xl10911054 style='border-right:1.0pt solid black;
  height:15.0pt;border-left:none'>Phone Number:<span style='mso-spacerun:yes'> <input style="background-color: #FFFFFF" name="locationcontactphonenumber" id="locationcontactphonenumber" type="tel" /></span></td>
 </tr>
 <tr height=20 style='mso-height-source:userset;height:15.0pt'>
...
/* <!-- Check for constant elements that are empty --> */
    ...
    var issuerlocation = document.getElementById("issuerlocation").value;
...
if( requestlocation_chk == 0 ||siterefnumber_chk == 0 ||cmsrefnumber_chk == 0 ||orderdate_chk == 0 ||locationcontactperson_chk == 0 ||locationcontactphonenumber_chk == 0 ||producttype_chk == 0 ||orderstatus_chk == 0||issuedby_chk == 0||issuedate_chk == 0){
 ...
 /* <!-- Insert if brandname is set --> */ 
            for(i=0; i<select.length;i++){
                ...
                if(brandname && brandname != ''){
                    updateOrdersTable(orderdate,manufacturer,locationcontactperson,locationcontactphonenumber,producttype,siterefnumber,authorisedby,dateauthorized,remarks,quantity,orderstatus,productorderid,lotno,issuedby,issuedate,brandname,requestlocation,cmsrefnumber,issuerlocation);
                }
            }
...
//Get Element ID				
				...
				var weight = 'weight'+i;
                var productorderid = 'productorderid'+i;
...
//Set Total Weight to int value 0 to enable calculations 
			...
			$.ajax({
            url:"{{url('get-product-recipient-by-type')}}",
            type: "POST",
            data: {
...
var totalweight = 0;
 ...
 document.getElementById("requestlocation").addEventListener('change', function(ev){
    var today = new Date().toISOString()
    document.getElementById("siterefnumber").value = ev.target.value;
    document.getElementById("cmsrefnumber").value = ev.target.value+today;
...
//create element values
	...
	var productorderid = 'productorderid'+elementid;
...
//reset all field elements
	...
	var issuerlocation = document.getElementById("issuerlocation").value;
	$.ajax({
        /* The Product LotNo is actually the productOrderId */
            url:"{{url('get-product-recipient-by-lotno')}}",
            type: "POST",
            data: {
                productlotno: productlotno, /* NOTE:  The Product LotNo is actually the productOrderId */
                issuerlocation: issuerlocation,
            _token: '{{csrf_token()}}' 
            },
            dataType : 'json',
...
//Set element values with Database data
						...
						document.getElementById(productorderid).value=value.productid;
...
function getQuantityAvailableBylotno(productlotno,quantityIssued,elementid){
	...
	var productorderid = 'productorderid'+elementid;
    var issuerlocation = document.getElementById("issuerlocation").value;
    var productid = document.getElementById(productorderid).value;
	$.ajax({
            url:"{{url('compare-recipient-quantity-available')}}",
            type: "POST",
            data: {
                productlotno: productlotno,
                issuerlocation: issuerlocation,
                productorderid: productid, /* comparison is done using productOrderId */
            _token: '{{csrf_token()}}' 
            },
            dataType : 'json',
            success: function(result){
...
//Call notification function to alert on Quantity remaining for stock
 ...
 /* <!-- Insert Function to commit data into orders table. All dynamic order variables are passed --> */ 
   ...
   function updateOrdersTable(orderdate,manufacturer,locationcontactperson,locationcontactphonenumber,producttype,siterefnumber,authorisedby,dateauthorized,remarks,quantity,orderstatus,productorderid,lotno,issuedby,issuedate,brandname,requestlocation,cmsrefnumber,issuerlocation){
            $.ajax({
            url:"{{url('issue-recipient-order-request')}}",
            type: "POST",
            data: {
                issuerlocation: issuerlocation,
                orderdate: orderdate,
            ...
            success: function(result){
				if(result.status == 1){
					...
					updateStockQuantity(quantity,lotno,productorderid);
					insertIntoRecipientsTable(brandname,requestlocation,quantity,lotno,issuedate,cmsrefnumber,issuerlocation);
				}
				else{
	...
	function insertIntoRecipientsTable(brandname,requestlocation,quantity,lotno,issuedate,cmsrefnumber,issuerlocation){
		$.ajax({
            url:"{{url('insert-into-final-recipients-stock-table')}}",
            type: "POST",
            data: {
                issuerlocation: issuerlocation,
                cmsrefnumber: cmsrefnumber,
	...
	function updateStockQuantity(quantity,lotno,productorderid){
        var issuerlocation = document.getElementById("issuerlocation").value;
		$.ajax({
            url:"{{url('update-recipient-stock-remainingquantity')}}",
            type: "POST",
            data: {
                quantity: quantity,
                lotno: lotno,
                issuerlocation: issuerlocation,
                productorderid: productorderid, /* Update on remaining QTY is done using productorderid */
            _token: '{{csrf_token()}}' 
            },
            dataType : 'json',
            success: function(result){
...
for(i=0; i<20; i++){
   ...
   for(x = L; x >= 0; x--) {
    console.log('Cleaning: '+brandname4clear)
